...
for (N in 1:length(plates)) {
  ...
  for (i in 1:length(gdl)) {
    ...
    snv_table <- as.data.frame(matrix(0, nrow = 4*29, ncol = 1))
    colnames(snv_table) <- plate
    rownames(snv_table) <- paste0(rep(c(-23:-1, 1:6), each=4), c('A', 'T', 'C', 'G'))
    DI_table <- data.frame(NULL)
    
    alns <- 0
    names(alns) <- plate
    cat(paste0(plate, '_', gname, '...\n'))
      
    bam_fnames <- dir('bam', pattern = '.bam$', full.names = TRUE)
    sample_names <- sub('.bam', '', dir('bam', pattern = '.bam$'))
    ...
    if (!is.null(crispr_set)) {
      pdf(paste0(folder, '/', plate, '_', gname, '.pdf'), wid=15, hei=15)
      if (pam == 'Cas9') {
      ...
      if (length(DI) > 0){
        ...
        if (nrow(DI_table) == 0){
          ...
          DI_table <- rbind(as.matrix(DI_table), as.matrix(DI_freqs))
        }
      }
    ...
    write.table(DI_table, paste0(output_DI, '_', plate, '_', gname, '.tsv'), col.names = NA, quote=F, sep='\t')
  }
}
